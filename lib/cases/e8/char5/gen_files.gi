...
# Read(Filename(e8_dir_char5,"init_char5.gi")); Read(Filename(e8_dir_char5,"gen_files.gi"));
#

for l in labels do
    f:=Filename(e8_dir_char5,Concatenation(l,".gi"));
    #PrintTo(f,l);
    PrintTo(f,"#\n");
    AppendTo(f,"# usage:\n");
    AppendTo(f,"# Read(\"~/Workspace/Chevalley.gap/init.gi\"); Read(Filename(home_dir,\"load.gi\")); ");
    AppendTo(f,"Read(Filename(e8_dir_char5,\"init_char5.gi\")); Read(Filename(e8_dir_char5,\"");
